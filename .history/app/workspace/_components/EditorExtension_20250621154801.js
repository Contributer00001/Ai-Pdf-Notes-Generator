...
function EditorExtension({ editor }) {
    ...
    const onAiClick = async () => {
        // 1. Get selected text from the editor
        ...
        // 2. Use Gemini to turn the question into smart search keywords
        const keywordPrompt = `
You are an assistant that extracts meaningful keywords for document search.

Question:
"${selectedText}"

Return only keywords or short phrases, comma-separated, no explanation.
`;
        const keywordified = await chatSession.sendMessage(keywordPrompt);
        const keywordQuery = keywordified.response.text().replace(/\n/g, ' ').trim();
        console.log("Keywordified Query:", keywordQuery);

        // 3. Search with the enhanced keyword query
        ...
        // 5. Construct final HTML answer prompt
        const PROMPT = `
...
`;
        ...
        // 6. Ask Gemini to generate answer in HTML
