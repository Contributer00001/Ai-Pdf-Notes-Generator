...
const onAiClick = async () => {
  ...
  let UnformattedAns;
  try {
    UnformattedAns = JSON.parse(searchResponse);
  } catch (e) {
    console.error("Error parsing search result:", searchResponse);
    editor.commands.insertContent(
      `<p><i>Something went wrong fetching related content.</i></p>`
    );
    return;
  }
  ...
  if (!AllUnformattedAns.trim()) {
    editor.commands.insertContent(
      `<p><i>Sorry, no relevant content was found in the PDF for your query.</i></p>`
    );
    return;
  }
...
`;
  ...
  // Step 4: Clean up markdown wrappers if any
  let FinalAns = rawResponse
    ?.replace(/^```html/i, '')
    ?.replace(/```$/, '')
    ?.trim();

  if (!FinalAns) {
    FinalAns = `<p><i>Sorry, something went wrong generating the answer.</i></p>`;
  }

  // Step 5: Append answer to editor content
